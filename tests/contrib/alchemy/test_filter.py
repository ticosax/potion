...
class FilterTestCase(BaseTestCase):
    def setUp(self):
        ...
        class Thing(sa.Model):
            ...
            belongs_to = sa.relationship(
                User, backref=backref('things', lazy='dynamic')
            )
    ...
    def test_equality(self):
        ...
        self.assertEqualWithout(
            ...
            response.json,
            without=['$uri', '$uri', '$id', '$type', 'gender', 'age', 'is_staff'],
        )
    ...
    def test_sort_relationship(self):
        ...
        thinggetter = (
            lambda thing: thing['belongs_to']['$ref'] if thing['belongs_to'] else ''
        )
        ...
        self.assertEqual(
            list(reversed(sorted(response.json, key=thinggetter))), response.json
        )

        response = self.client.get('/thing?sort={"name": false, "belongs_to": false}')
        self.assertEqual(
            sorted(response.json, key=lambda thing: thing['name']), response.json
        )
    ...
    def test_filter_and_sort_uri(self):
        for thing in [{"name": "A thing"}, {"name": "B thing"}]:
            response = self.client.post('/thing', data=thing)
            self.assert200(response)
    ...
    def test_sort_filter_date(self):
        ...
        self.assertEqual(
            sorted(response.json, key=lambda thing: thing['date_time']['$date']),
            response.json,
        )
