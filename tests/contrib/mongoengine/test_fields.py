...
class FieldsTestCase(MongoEngineTestCase):
    ...
    def test_map_field(self):
        ...
        response = self.client.get('/model/schema')
        self.assertEqual(
            response.json['properties']['float_mapping']['additionalProperties'],
            {'type': 'number'},
        )
        self.assertEqual(response.json['properties']['float_mapping']['default'], {})
        self.assertEqual(
            response.json['properties']['string_mapping']['additionalProperties'],
            {'type': 'string'},
        )
        self.assertEqual(response.json['properties']['string_mapping']['default'], {})
        self.assertEqual(
            response.json['properties']['reference_mapping']['additionalProperties'],
            {
        ...
        response = self.client.post(
            "/model",
            data={
                'reference_mapping': {"1": ref_model},
                'object_id_mapping': {"a": ref_model["id"]},
            },
        )
        self.assertEqualWithout(
            response.json,
            {
                'reference_mapping': {'1': {'id': 'adfb48a6763c5741b92f6ade'}},
                '$uri': '/model/561e6a807f551ce7d5df1003',
                'float_mapping': {},
                'object_id_mapping': {'a': 'adfb48a6763c5741b92f6ade'},
                'string_mapping': {},
            },
            without=["$uri"],
        )
