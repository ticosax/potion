...
class ModelResourceTestCase(BaseTestCase):
    ...
    def test_schema_io(self):
        ...
        response = self.client.post(
            "/foo", data={"name": "Foo", "secret": "mystery", "slug": "foo"}
        )

        self.assert200(response)
        self.assertEqual(
            {"$uri": "/foo/1", "name": "Foo", "slug": "foo"}, response.json
        )

        self.assertEqual(
            {"id": 1, "name": "Foo", "slug": "foo", "secret": "mystery"},
            FooResource.manager.items[1],
        )

        response = self.client.patch("/foo/1", data={"name": "Bar", "secret": "123456"})

        self.assert400(response)

        response = self.client.patch("/foo/1", data={"name": "Bar"})

        self.assert200(response)

        self.assertEqual(
            {"$uri": "/foo/1", "name": "Bar", "slug": "foo"}, response.json
        )

        self.assertEqual(
            {"id": 1, "name": "Bar", "slug": "foo", "secret": "mystery"},
            FooResource.manager.items[1],
        )
    ...
    def test_schema_io_create_flag(self):
        ...
        [create_link] = [link for link in data['links'] if link['rel'] == 'create']
        [update_link] = [link for link in data['links'] if link['rel'] == 'update']
        ...
        self.assertEqual(["$uri", "name", "slug"], sorted(data["properties"].keys()))
